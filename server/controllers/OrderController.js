...
// Create orders from subscription meals
export const createOrdersFromSubscription = async (subscriptionId) => {
  try {
    console.log(`📦 Creating orders for subscription: ${subscriptionId}`);

    // Find the subscription with populated data
    ...
    for (const meal of subscription.mealSelections) {
      try {
        ...
        if (!menuItem) {
          errors.push(`Menu item not found: ${meal.menuItemId}`);
          continue;
        }
        ...
        const order = new Order({
          ...
          total: meal.price * (meal.quantity || 1),
          deliveryDateTime,
          deliveryAddress: {
    ...
    // Save subscription with updated order IDs
    ...
    console.error("Error creating orders from subscription:", error);
    return {
      success: false,
      error: error.message
    };
...
// API endpoint to trigger order creation for a subscription
export const createOrdersForSubscription = async (req, res) => {
  try {
    const { subscriptionId } = req.params;
    
    const result = await createOrdersFromSubscription(subscriptionId);
    
    if (result.success) {
      res.json({
        success: true,
        message: `Created ${result.createdOrders} orders from subscription`,
        data: result
      });
    } else {
      res.status(500).json({
        success: false,
        message: result.error || "Failed to create orders"
      });
    }
  } catch (error) {
    res.status(500).json({
...
// Cancel orders for a subscription (used when pausing/cancelling)
export const cancelSubscriptionOrders = async (subscriptionId, fromDate) => {
  try {
    const query = {
      subscriptionId,
      status: 'pending'
    ...
    if (fromDate) {
      query.deliveryDateTime = { $gte: fromDate };
    }
    ...
    console.error("Error cancelling subscription orders:", error);
    return {
      success: false,
      error: error.message
    };
...
export const createOrder = async (req, res) => {
  try {
    ...
    const order = await Order.create({
      ...
      subscriptionId,
      paymentMethod: isSubscription==='sslcommerz', 
